...
## 2026-02-27 - 实时检测预览页新增信息模块（减少空感）

### 需求
- 用户反馈页面“还是有点空”，希望不是只改高度，而是增加有意义的内容。

### 完成内容
- 在 `LiveScoring` 预览页底部说明区新增两块实用模块：
   - **当前准备状态**：标准视频、摄像头、实时画面、启动条件的就绪状态。
   - **检测流程**：预览确认 → 开始检测 → 实时对比 → 结束查看结果。
- 将底部说明区最小高度从 `180px` 调整为 `220px`，与新增模块匹配。
- 保持页面功能逻辑不变，仅增强信息密度和视觉完整度。
- 前端构建验证通过。
...
### 潜在问题
- 在较窄屏幕下卡片信息会换行，属响应式正常表现。

## 2026-02-27 - 实时检测页底部空白修正

### 需求
- 实时检测页面仍有明显空白区域，页面看起来发空。

### 完成内容
- 将 `LiveScoring` 外层容器改为 `min-h-screen`，确保页面背景至少铺满整个视口。
- 保持现有布局与模块不变，仅做最小高度修正。
- 前端构建验证通过。
...
### 潜在问题
- 如果后续在小屏设备仍感到内容稀疏，可再微调视频区和说明区高度比例。

## 2026-02-27 - 体态检测页背景轻量缩短

### 需求
- 当前体态检测页面背景区域偏长，希望稍微缩短一点。

### 完成内容
- 将页面根容器最小高度从 `min-h-screen` 调整为 `min-h-[94vh]`。
- 将主内容上下内边距从 `py-8` 调整为 `py-6`。
- 保持页面结构与功能不变，仅做轻量视觉收紧。
- 前端构建验证通过。
...
### 潜在问题
- 在极小屏幕下，内容较多时仍可能出现滚动，这是正常现象。

## 2026-02-27 - 体态检测自动拍照页重排与摄像头选择
...
### 完成内容
- 自动拍照页面布局改为：上方大画面（AutoCaptureGuide）+ 下方平铺拍摄进度卡片。
- 拍摄进度从右侧竖排改为底部横向平铺（响应式 1/2/4 列）。
- 在自动拍照区域新增摄像头下拉选择和刷新按钮，支持切换摄像头设备。
- 新增摄像头设备枚举与切换逻辑：
   - 页面初始化加载可用摄像头列表
   - 启动自动拍照时按所选设备打开摄像头
   - 自动拍照过程中可切换摄像头并热更新流
...
### 潜在问题
- 个别浏览器在未授权前可能拿不到摄像头设备标签，用户授权后刷新一次可显示完整名称。

## 2026-02-27 - 移除体态检测页头部横幅

### 需求
- 不再需要体态检测页面顶部头部横幅。

### 完成内容
- 删除 `PostureAssessment` 页面顶部导航头部（标题与右侧快捷按钮）。
- 主内容区从 `pt-24 pb-8` 调整为 `py-8`，消除头部移除后的空白。
- 前端构建验证通过。
...
### 潜在问题
- 原头部中的快捷入口（返回首页/历史/个人中心）已移除，若需要可在内容区补充入口按钮。

## 2026-02-27 - 体态检测页头部固定吸顶

### 需求
- 体态检测页面头部横幅需要始终固定在页面最上方，不随滚动移动。

### 完成内容
- 将 `PostureAssessment` 顶部导航从 `sticky` 改为 `fixed top-0 left-0 right-0`。
- 提升层级为 `z-[80]`，确保导航在页面内容上层显示。
- 主内容区增加顶部留白 `pt-24`，避免固定头部遮挡正文。
- 前端构建验证通过。
...
### 潜在问题
- 若未来全局还有更高层级悬浮组件（如全局弹层），需统一 z-index 规范以避免遮挡冲突。

## 2026-02-27 - 预览页双视频高度微调

### 需求
- 两个预览视频面板希望“再高一点点”。

### 完成内容
- 将非全屏预览区两个视频容器最大高度从 `34vh` 微调到 `37vh`。
- 保持 16:9 比例与其余布局不变，仅做轻量高度提升。
- 前端构建验证通过。
...
### 潜在问题
- 若继续上调高度，可能再次挤压下方说明区空间。

## 2026-02-27 - 预览页空白区域填充（说明卡片）

### 需求
- 预览页下方仍有较大空白区域，希望增加内容以提升完整性。

### 完成内容
- 在非全屏双预览画面下方新增“预览说明区”。
- 说明区包含两部分：
   - 预览说明（页面用途与开始检测前的操作提示）
   - 开始前检查清单（构图、机位、光线、标准视频）
- 保持“预览页”定位，不引入实时评分数据。
- 前端构建验证通过。
...
### 潜在问题
- 若后续改为极简风格，可将说明区折叠成可展开模块。

## 2026-02-27 - 实时检测页回归预览模式并缩短页面高度

### 需求
- 当前页面仅用于预览，不需要展示实时统计数据。
- 页面整体偏长，希望缩短并更紧凑。

### 完成内容
- 移除非全屏模式下新增的底部实时状态面板（分数/FPS/延迟等）。
- 去掉实时画面卡片中的分数徽标，保持预览页简洁。
- 缩短页面纵向高度：减小容器间距与内边距，视频容器高度从 `max-h-[42vh]` 调整为 `max-h-[34vh]`。
- 保持 16:9 预览比例与无黑边填充策略不变。
- 完成前端构建验证，编译通过。
...
### 潜在问题
- 如果后续在小屏设备上需要更大预览面积，可按断点单独提高移动端 `max-h`。

## 2026-02-27 - 实时检测页底部留白填充优化

### 需求
- 实时检测页面下方留白过大，视觉不舒适。

### 完成内容
- 在非全屏模式两个视频卡片下方新增“实时状态面板”，用于填充剩余空间。
- 状态面板展示：当前分数、平均分、FPS、已处理帧、检测状态与延迟。
- 面板采用与现有页面一致的深色卡片样式，保持视觉统一。
- 完成前端构建验证，编译通过。
...
### 潜在问题
- 小屏设备下统计卡片可能换行较多；如需更紧凑，可后续改为横向滚动统计条。

## 2026-02-27 - 实时检测画面改为 16:9 并消除上下黑边
...
### 完成内容
- 实时检测普通模式下，双画面容器统一改为 `aspect-video`（16:9）。
- 双画面 `LiveVideoPanel` 新增 `fitMode` 属性，并在实时检测页设置为 `cover`，让画面铺满容器，消除上下黑边。
- 保持现有卡片样式和交互不变，仅调整渲染比例与填充策略。
- 已执行前端构建验证，构建通过。
...
### 潜在问题
- `cover` 会对部分非 16:9 源进行边缘裁切；若后续更看重完整取景，可按场景切回 `contain`。

## 2026-02-27 - 实时检测视频容器与摄像头下拉样式优化

### 需求
- 实时画面上下黑边过大，视频内容看起来只占中间一小块。
- 摄像头切换下拉框希望更美观，和顶部工具栏风格更统一。

### 完成内容
- 将实时检测普通模式下的双视频容器改为固定比例卡片（`4:3`），并限制最大高度（`max-h-[44vh]`），减少上下黑边视觉占比。
- 调整双卡片布局为 `items-start` 的紧凑网格，去掉卡片强制拉伸高度，避免容器过高导致空黑区域放大。
- 重做摄像头下拉框样式：加入左侧摄像头图标、右侧下拉箭头、半透明背景、悬停与聚焦状态增强。
- 将刷新按钮并入下拉框内部右侧，视觉更统一，仍保留原有刷新功能。
- 完成前端构建验证，页面可正常编译。
...
### 潜在问题
- 固定为 `4:3` 后，若标准视频是超宽比例，可能会出现左右留黑；若后续要进一步提升适配，可改为按视频源动态比例渲染。
